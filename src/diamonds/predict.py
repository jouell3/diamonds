...
from diamonds.registry import load_model_mkflow, load_model
...
def predict_values(X: pd.DataFrame) -> dict[str, float]:
    ...
    preproc = load_model("preproc.pkl")
    prod_model = load_model_mkflow()
    ...
    y_pred = predict(prod_model, X_scaled)
